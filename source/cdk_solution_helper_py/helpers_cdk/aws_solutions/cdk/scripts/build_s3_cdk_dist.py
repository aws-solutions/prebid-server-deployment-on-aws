...
def validate_version_code(ctx, param, value):
    ...
    """
    re_prerelease_ver = r'0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*'
    re_semver = rf"^v(?P<major>0|[1-9]\d*)\.(?P<minor>0|[1-9]\d*)\.(?P<patch>0|[1-9]\d*)(?:-(?P<prerelease>(?:{re_prerelease_ver})(?:\.(?:{re_prerelease_ver}))*))?(?:\+(?P<buildmetadata>[0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$"
...
def source_code_package(ctx, ignore, solution_name):
    ...
    # copy the required run-unit-tests.sh, venv_check.py, build-s3-dist.sh
    (Path(env.open_source_dir) / "deployment").mkdir()

    for copy_file in ["run-unit-tests.sh", "venv_check.py", "build-s3-dist.sh"]:
        try:
            shutil.copyfile(
                Path(env.template_dir) / copy_file,
                Path(env.open_source_dir) / "deployment" / copy_file,
            )
        except FileNotFoundError:
            raise click.ClickException(
    ...
    try:
        shutil.copytree(
            Path(env.template_dir) / "ecr",
            Path(env.open_source_dir) / "deployment" / "ecr",
        )
    except FileNotFoundError:
        raise click.ClickException(
